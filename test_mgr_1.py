...
def etopsis_method(dataset, weights, epsilon, criterion_type, 
    ...
    #for i in range(0, dataset.shape[0]):
        #v_ij_row = np.sum(v_ij[i])
        #r_ij_row = np.sum(r_ij[i])
        #WM = v_ij_row/sum_w
        #WSD = (np.sum((r_ij_row-WM)**(2)))/num_rows
        #WM_WSD = (WM**(2) + WSD**(2))**(1/2)
        #w_WM_WSD = ((mean_w - WM)**(2) + WSD**(2))**(1/2)
        #R[i] = WM_WSD/(WM_WSD + w_WM_WSD)
        #A = (WM**(2)+(WSD/epsilon)**(2))**(1./2)
        #I = ((mean_w-WM)**(2)+(WSD/epsilon)**(2))**(1./2)
        #R[i] = A/(A+I)
